...
def PlayMRC(): #95 Play filmes
	...
	try:
		...
		if player:
			...
			mp4 = common.OpenURL(player ,headers={'referer': "https://canaisgratis.org"})
			...
			#url2 = m[0]
			#file = mp4[0][1]+".mp4"
			AddDir("[B][COLOR yellow]"+ name +" [/COLOR][/B]"  , file[0] + "|Referer=https://canaisgratis.org/", 3, iconimage, iconimage, index=0, isFolder=False, IsPlayable=True, info=desc, background=url+";;;"+name+";;;RC")
		else:
...
def PlayMRC2(): #96 Play filmes
	...
	try:
		...
		if player:
			...
			PlayUrl("[B][COLOR white]"+ name +" [/COLOR][/B]", file[0] + "|Referer=https://canaisgratis.org/", iconimage, desc) #aqui
		else:
			AddDir("[B]Ocorreu um erro[/B]"  , "", 0, iconimage, iconimage, index=0, isFolder=False, IsPlayable=False, info="Erro")
	except:
		AddDir("Server error, tente novamente em alguns minutos" , "", 0, "", "")
...
def PlaySRC(): #133 Play series
	try:
		...
		if player:
			...
			mp4 = common.OpenURL(player ,headers={'referer': "https://canaisgratis.org"})
			file=re.compile('[^"|\']+\.mp4').findall(mp4)
			PlayUrl(name, file[0] + "|Referer=https://canaisgratis.org/", iconimage, name)
		else:
			xbmcgui.Dialog().ok('Play XD', 'Erro, tente novamente em alguns minutos')
	except:
		xbmcgui.Dialog().ok('Play XD', 'Erro, tente novamente em alguns minutos')
