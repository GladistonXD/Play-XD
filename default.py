...
def Series(): #60
	try:
		...
		lista = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(l2[0]+l3[0]+l4[0]+l5[0]+l6[0])
		if cOrdNCS=="1":
...
def MoviesNC(): #71
	...
	try:
		...
		if CatGO=="4":        
			...
			lista = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(l48[0])            
		if CatGO=="5":        
			...
			lista = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(l49[0]+l50[0]+l51[0]+l52[0]+l53[0]+l54[0]+l55[0]+l56[0]+l57[0]+l58[0]+l59[0]+l60[0])
		if CatGO=="6":        
			...
			lista = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(l61[0]+l62[0]+l63[0])
		if CatGO=="7":        
			...
			lista = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(l64[0]+l65[0]+l66[0]+l67[0])
		if CatGO=="8":        
			link = common.OpenURL("https://netcine.info/category/faroeste/").replace('\n','').replace('\r','')
			l68 = re.compile("box_movies(.+)").findall(link)
			lista = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(l68[0])
		if CatGO=="9":        
		...
		if CatGO=="10":        
			...
			lista = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(l72[0]+l73[0]+l74[0]+l75[0]+l76[0]+l77[0])
		if CatGO=="11":        
			...
			lista = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(l78[0]+l79[0]+l80[0]+l81[0])       
			lista = sorted(lista, key=lambda lista: lista[1])
		for img2,name2,url2 in lista:
			if name2!="Close":
				name2 = name2.replace("&#8211;","-").replace("&#038;","&").replace("&#8217;","\'")
...
def PlayMRC(): #95 Play filmes
	...
	try:
		...
		if player:
			...
			mp4 = common.OpenURL(player ,headers={'referer': "https://canaisgratis.info"})
			...
			#url2 = m[0]
			#file = mp4[0][1]+".mp4"
			AddDir("[B][COLOR yellow]"+ name +" [/COLOR][/B]"  , file[0] + "|Referer=https://canaisgratis.info/&User-Agent=Mozilla/5.0 (Windows NT 10.0 Win64 x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.45 Safari/537.36 Edg/79.0.309.30", 3, iconimage, iconimage, index=0, isFolder=False, IsPlayable=True, info=desc, background=url+";;;"+name+";;;RC")
		else:
...
def PlayMRC2(): #96 Play filmes
	...
	try:
		...
		if player:
			...
			PlayUrl("[B][COLOR white]"+ name +" [/COLOR][/B]", file[0] + "|Referer=https://canaisgratis.info/&User-Agent=Mozilla/5.0 (Windows NT 10.0 Win64 x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.45 Safari/537.36 Edg/79.0.309.30", iconimage, desc) #aqui
		else:
			AddDir("[B]Ocorreu um erro[/B]"  , "", 0, iconimage, iconimage, index=0, isFolder=False, IsPlayable=False, info="Erro")
	except:
		AddDir("Server error, tente novamente em alguns minutos" , "", 0, "", "")
...
def PlaySRC(): #133 Play series
	try:
		...
		if player:
			...
			mp4 = common.OpenURL(player ,headers={'referer': "https://canaisgratis.info"})
			file=re.compile('[^"|\']+\.mp4').findall(mp4)
			PlayUrl(name, file[0] + "|Referer=https://canaisgratis.info/&User-Agent=Mozilla/5.0 (Windows NT 10.0 Win64 x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.45 Safari/537.36 Edg/79.0.309.30", iconimage, name)
		else:
			xbmcgui.Dialog().ok('Play XD', 'Erro, tente novamente em alguns minutos')
	except:
		xbmcgui.Dialog().ok('Play XD', 'Erro, tente novamente em alguns minutos')
...
def PlayTVCB(): #103
	...
	PlayUrl(name, m[0] + "|Referer=https://canaisgratis.info/&User-Agent=Mozilla/5.0 (Windows NT 10.0 Win64 x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.45 Safari/537.36 Edg/79.0.309.30", iconimage, name, "")
