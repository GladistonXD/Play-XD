...
def PlayTVCB(): #103
	...
	player = re.compile('https://canaisgratis.org/' + '<iframe.name="Player" ="" src="\/([^\"]+)').findall(link)
	#player = re.sub('.php', "vibgratis.php", player[0] )
	#player = re.sub('^/', player[0] )
...
def PlayTVRC(): # 101
	...
	try:
		...
		link3 = common.OpenURL("http://canaisgratis.org/player3/canaisplayer.php?canal="+player[0],headers={'referer': "http://social.olimpicusads.com/"})
		ST(link3)
		#ST(link3)
		...
		if urlp:
			#ST(urlp[0])
			PlayUrl(name, urlp[0] + "|Referer=http://canaisgratis.org/player3/canaisvibfree.php?canal=", iconimage, info)
		else:
