...
def MFilmes(): #-2
	...
	AddDir("[COLOR lightgreen][B]Filmes Max[/B][/COLOR]" , "", 465, "https://uploaddeimagens.com.br/images/002/428/080/original/ROBOZIm.jpg", "https://uploaddeimagens.com.br/images/002/428/080/original/ROBOZIm.jpg", info='[COLOR][/COLOR]')
	AddDir("[COLOR indianred][B]Filmes Assistir.One[/B][/COLOR]" , "", 458, "https://uploaddeimagens.com.br/images/002/640/063/original/Vin.png", "https://uploaddeimagens.com.br/images/002/640/063/original/Vin.png", info='[COLOR][/COLOR]')
	setViewM()
def MSeries(): #-3
	...
	AddDir("[B][COLOR lightgreen]Séries Max[/COLOR][/B]", "config" , 468,"https://walter.trakt.tv/images/shows/000/037/522/fanarts/thumb/6ecdb75c1c.jpg", "https://walter.trakt.tv/images/shows/000/037/522/fanarts/thumb/6ecdb75c1c.jpg", isFolder=True, info='[COLOR][/COLOR]')
	...
	AddDir("[B][COLOR indianred]Séries Assistir.One[/COLOR][/B]", "config" , 461,"https://www.jornadageek.com.br/wp-content/uploads/2019/01/Vikings-5%C2%AA-temporada.jpg", "https://www.jornadageek.com.br/wp-content/uploads/2019/01/Vikings-5%C2%AA-temporada.jpg", isFolder=True, info='[COLOR][/COLOR]')
...
def MaxMenuSerie(): # 468
	...
	for x in range(0, 2):
		try:
			y +=1
			link = common.OpenURL("https://maxseries.me/series-e-animes/page/"+str(y))
			...
			if match:
				for url2, img2, name2 in match:
					...
					AddDir(name2, url2, 469, img2, isFolder=True, IsPlayable=True, info='[COLOR][/COLOR]')
		except:
			pass
...
def MaxMenu(): # 465
	...
	try:
		...
		for x in range(0, 2):
			l +=1
			link = common.OpenURL("https://maxfilmes.me/"+ClistaMX10[int(CatMX)]+"/page/"+str(l))
			match = re.compile('"location.href=.(.+?).;.+?src="(.+?)".alt="(.+?)"').findall(link)
			if match:
				for url2, img2, name2 in match:
					img2 = img2.replace("w185","original")
					name2 = name2.replace('&#8217;','’').replace('&#8211;','–').replace('&#038;','&').replace('&#8216;','‘').replace('&#8220;','“').replace('&#8221;','”').replace('&#8230;','…')
					AddDir(name2, url2, 466, img2, isFolder=True, IsPlayable=True, info='[COLOR][/COLOR]')
					p += 1
		if p >= 48:
...
def AssistirMenu(): # 458
	AddDir("[COLOR yellow][B][Genero dos Filmes]:[/B] " + ClistaAS11[int(CatAS)] +"[/COLOR]", "url" ,236 ,"https://lh5.ggpht.com/gv992ET6R_InCoMXXwIbdRLJczqOHFfLxIeY-bN2nFq0r8MDe-y-cF2aWq6Qy9P_K-4=w300", "https://lh5.ggpht.com/gv992ET6R_InCoMXXwIbdRLJczqOHFfLxIeY-bN2nFq0r8MDe-y-cF2aWq6Qy9P_K-4=w300", isFolder=False, info='[COLOR][/COLOR]')
	try:
		p= 1
		if int(cPageAS) > 0:
		...
		for x in range(0, 1):
			l +=1
			link = common.OpenURL("https://assistir.one/lancamentos/"+str(l)).replace('\n','').replace('\r','')
			match = re.compile('span>                             <img src=(.+?)".+?href="(.+?)".+?title="(.+?)"').findall(link)
			if ClistaAS10[int(CatAS)] != "0":
				link = common.OpenURL("https://assistir.one/categoria/"+ClistaAS10[int(CatAS)]+"/filmes/"+str(l)).replace('\n','').replace('\r','')
				match = re.compile('>                            <img src=(.+?)".+?href="(.+?)".+?title="(.+?)"').findall(link)
			if match:
				for img2,url2, name2 in match:
					img3 = "https:" + img2
					url3 = "https:" + url2
					AddDir(name2, url3, 459, img3.replace('https:"https:',"https:").replace('"',""), img3.replace('https:"https:',"https:").replace('"',""), isFolder=True, IsPlayable=True, info='[COLOR][/COLOR]')
					p += 1
			else:
				break
		if p >= 48:
...
def AssistirMenu3(): # 460
	try:    
		...
		url4 = re.compile('value=.(.+?)".+?(Player.+?)<').findall(url3x.text.encode('utf8'))
...
def AssistirSeries(): # 461
	...
	for x in range(0, 2):
		try:
			y +=1
			link = common.OpenURL("https://assistir.one/series/"+str(y)).replace('\n','').replace('\r','')
			match = re.compile('span>                             <img src=(.+?)".+?href="(.+?)".+?title="(.+?)"').findall(link)
			if match:
				for img2,url2, name2 in match:
					img3 = "https:" + img2
...
def AssistirSeries3(): # 463
	...
	temp = re.compile('-(.+)').findall(url.encode('utf8'))
...
def AssistirSeriesPlay (): # 464
	try:	
		id = re.compile('https:..(.+?)-').findall(url.encode('utf8'))
		idl = re.compile('-(.+?)#').findall(url.encode('utf8'))
		item = re.compile('#(.+)').findall(url.encode('utf8'))
...
def Series(): #60
	try:
		...
		try:
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/1/")#,headers=headers, proxies=proxies)
			l2 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/2/")#, headers=headers, proxies=proxies)
			l3 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/3/")#, headers=headers, proxies=proxies)
			l4 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/4/")#, headers=headers, proxies=proxies)
			l5 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/5/")#, headers=headers, proxies=proxies)
			l6 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/6/")
			l7 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/7/")
			l8 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/8/")
			l9 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/9/")
			l10 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/10/")
			l11 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/11/")
			l12 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/12/")
			l13 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/13/")
			l14 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))
			link = requests.get("http://138.122.11.44:443/?url="+"http://netcine.biz/tvshows/page/14/")
			l15 = re.compile("box_movies(.+)").findall(link.text.encode('utf-8').replace('\n','').replace('\r',''))        
			lista = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(l2[0]+l3[0]+l4[0]+l5[0]+l6[0]+l7[0]+l8[0]+l9[0]+l10[0]+l11[0]+l12[0]+l13[0]+l14[0]+l15[0])
			if cOrdNCS=="1":
				lista = sorted(lista, key=lambda lista: lista[1])
...
def MoviesNC(): #71 Netcine
	...
	try:
		...
		try:
			...
			for x in range(0, 2):
				l +=1
				link2 = common.OpenURL("http://138.122.11.44:443/?url="+"https://netcine.biz/"+ClistaGO0[int(CatGO)]+"/page/"+ str(l)+"/?filmes").replace('\n','').replace('\r','')
				m2 = re.compile("box_movies(.+)").findall(link2)
				lista2 = re.compile("img src\=\"([^\"]+).+?alt\=\"([^\"]+).+?f\=\"([^\"]+)").findall(m2[0])
				if lista2:
				 for img3,name3,url3 in lista2:
				  if name3!="Close" and name3!="NetCine":
					name3 = name3.replace("&#8211;","-").replace("&#038;","&").replace("&#8217;","\'")
					img3 = img3.replace("-120x170","")
				  if "tvshows" in url3: False
				  else:
					AddDir(name3,url3, 78, img3, img3, isFolder=True, IsPlayable=True, info='[COLOR][/COLOR]')
				p += 1
			if p >= 56:
				AddDir("[COLOR blue][B]Proxima Pagina >> ["+ str( int(cPageGOf) + 2) +"][/B][/COLOR]", cPageGOf , 110 ,"http://icons.iconarchive.com/icons/iconsmind/outline/256/Next-2-2-icon.png", isFolder=False, background="cPageGOf")
		except:
...
def ListMoviesNC(): #78
	try:
		...
		proxy = requests.get("https://raw.githubusercontent.com/GladistonXD/Filmes-2017/master/proxy")
		proxy2 = re.compile('proxy = "(.+?)"').findall(proxy.text)
		link = common.OpenURL("http://138.122.11.44:443/?url="+url).replace('\n','').replace('\r','')
		m = re.compile("\"play-.\".+?src=\"([^\"]+)").findall(link)
		m2 = re.compile("\#play-...(\w*)").findall(link)
		info2 = re.compile('<h2>Synopsis<\/h2>(.*?)<\/').findall(link)
		info2 = re.sub('<(.*?)>', '', info2[0] ) if info2 else ""
...
def PlayMRC2(): #96 Play filmes direto
	...
	try:
		...
		if player:
			...
			mp4 = common.OpenURL(player ,headers={'referer': 'https://gamesgo.fun/'})
			try:
				player = re.compile('href.{1,5}(mega[^"|\']*)').findall(mp4)
				mp42 = common.OpenURL("https://gamesgo.fun/player3/"+player[0] ,headers={'referer': 'https://gamesgo.fun/'})
...
#		pass
	...
	try:
		...
		for x in range(0, 1):
			l +=1
			link = common.OpenURL("https://maxfilmes.me/?s="+d.replace("%2B","+"))
			match = re.compile('class="thumbnail.+?"><img src="(.+?)" alt="(.+?)".+?href="(.+?)"').findall(link)
			if match:
				for img2, name2, url2 in match:
					img2 = img2.replace("w92","original")
					name2 = name2.replace('&#8217;','’').replace('&#8211;','–').replace('&#038;','&').replace('&#8216;','‘').replace('&#8220;','“').replace('&#8221;','”').replace('&#8230;','…')
					AddDir("[COLOR lightgreen]"+name2+"[/COLOR]", url2, 466, img2, isFolder=True, IsPlayable=True, info='[COLOR][/COLOR]')
	except:
		pass
	progress.update(93, "93%", "Max", "")        
	try:
		...
		for x in range(0, 1):
			l +=1
			link = common.OpenURL("https://maxseries.me/?s="+d.replace("%2B","+"))
			match = re.compile('class="thumbnail.+?"><img src="(.+?)" alt="(.+?)".+?href="(.+?)"').findall(link)
			if match:
				for img2, name2, url2 in match:
					img2 = img2.replace("w92","original")
					name2 = name2.replace('&#8217;','’').replace('&#8211;','–').replace('&#038;','&').replace('&#8216;','‘').replace('&#8220;','“').replace('&#8221;','”').replace('&#8230;','…')
					AddDir("[COLOR lightgreen]"+name2+"[/COLOR]", url2, 469, img2, isFolder=True, IsPlayable=True, info='[COLOR][/COLOR]')
	except:
		pass
	progress.update(95, "95%", "Assistir.One", "")        
	try:
		p= 1
		...
		for x in range(0, 1):
			l +=1
			link = common.OpenURL("https://assistir.one/ls.php?p=pesquisa&nome="+d.replace("%2B","%20")).replace('\n','').replace('\r','').replace("&nbsp;&nbsp;","")
...
def ListFilmeLancMM(): #184
	...
	try:
		...
		proxy = requests.get("https://raw.githubusercontent.com/GladistonXD/Filmes-2017/master/proxy")
		proxy2 = re.compile('proxy = "(.+?)"').findall(proxy.text)
		links = common.OpenURL("http://138.122.11.44:443/?url="+"http://www.mmfilmeshd.tv/series/")
		ms = re.compile('href\=\"(.+www.mmfilmeshd.tv.+)\" rel\=\"bookmark\"').findall(links)
		for x in range(0, 5):
			l+=1
			link = common.OpenURL("http://138.122.11.44:443/?url="+"http://www.mmfilmeshd.tv/ultimos/page/"+str(l)+"/")
			m = re.compile('id\=\"post\-\d+\".+?\=.([^\"]+)\h*(?s)(.+?)(http[^\"]+)').findall(link)
			res = re.compile('audioy..([^\<]*)').findall(link)
			jpg = re.compile('src=\"(http.+?www.mmfilmeshd.tv\/wp-content\/uploads[^\"]+)').findall(link)
			dubleg = re.compile('boxxer.+\s.+boxxer..([^\<]*)').findall(link)
...
def ListFilmeMM(pagina2): #180
	...
	try:
		...
		for x in range(0, 4):
			l+=1
			link = common.OpenURL("http://138.122.11.44:443/?url="+"http://www.mmfilmeshd.tv/"+ ClistaMM0[int(CatMM)] +"/page/"+str(l)+"/").replace('\n','').replace('\r','')
			m = re.compile('<li id=.+?" title="(.+?)".+?href="(.+?)".+?boxxer.+?boxxer">(.+?)<.+?src="(.+?)".+?audioy..(.+?)<').findall(link)
			if m:
				for name2, url2, dubleg, jpg, res in m:
					name2 = name2.replace("&#8211;","-").replace("&#038;","&").replace("&#8217;","\'").replace("&#8230;","")
...
def OpenLinkMM(): #181
	try:
		...
		info2= info2[0][0].replace("\t","") if info2 else ""
		if m:
			link2 = common.OpenURL("http://138.122.11.44:443/?url="+m[0],headers={'referer': "http://www.mmfilmeshd.tv/"})
			m2 = re.compile('opb\(.([^\']+).+?.{3}.+?[^\\>]+.([^\<]+)').findall(link2)
			if m2:
				name2 = re.sub(' \[.+', '', name )
				for link,dubleg in m2:
...
def PlayLinkMM(): #182
	try:
		...
		proxy = requests.get("https://raw.githubusercontent.com/GladistonXD/Filmes-2017/master/proxy")
		proxy2 = re.compile('proxy = "(.+?)"').findall(proxy.text)
		link = requests.get("http://138.122.11.44:443/?url="+url, headers={'referer': "http://www.mmfilmeshd.tv/"})
...
def ListSerieMM(): #190
	try:
		...
		proxy = requests.get("https://raw.githubusercontent.com/GladistonXD/Filmes-2017/master/proxy")
		proxy2 = re.compile('proxy = "(.+?)"').findall(proxy.text)
		link = common.OpenURL("http://138.122.11.44:443/?url="+"http://www.mmfilmeshd.tv/series/")
		m = re.compile('id\=\"post\-\d+\".+?\=.([^\"]+)\h*(?s)(.+?)(http[^\"]+)').findall(link)
		jpg = re.compile('src=\"(http.+?www.mmfilmeshd.tv\/wp-content\/uploads[^\"]+)').findall(link)
		i=0
		m2=[]
		...
		if m:
			if x=="None":
				...
				if d== -1:
					d= 0
				if m2:
					link3 = common.OpenURL("http://138.122.11.44:443/?url="+m2[0][0],headers={'referer': "http://www.mmfilmeshd.tv/"}).replace("\n","").replace("\r","").replace('".Svplayer"',"<end>").replace('\t'," ")
					link3 = re.sub('(\(s \=\= \d+\))', r'<end>\1', link3 )
					m3 = re.compile('s \=\= (\d+)(.+?\<end\>)').findall(link3)
					for temp in m3:
						AddDir("[B][Temporada "+ temp[0] +"][/B]" ,listal[d], 192, iconimage, iconimage, isFolder=True, info=info2, background=i)
...
def PlaySMM(): #194
	try:
		...
		if cdn:
			...
			link2 = common.OpenURL( re.sub('(\/.{1,25}\/).{1,10}\/', r'\1', "http://138.122.11.44:443/?url="+url) ,headers={'referer': "http://player.openload.network"}).replace('"',"'")
			m2 = re.compile('(h[^\']+).+?label...(\w+)').findall(link2)
			legenda = re.compile('([^\']+\.(vtt|srt|sub|ssa|txt|ass))').findall(link2)
			listar=[]
			listal=[]
